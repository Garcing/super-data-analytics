/*
普拉提初阶1期验证组（未使用agent）服务周期：
往期·人服比：   复购率：   期人效：
大盘·人服比：    复购率：    期人效：
验证组·人服比：  复购率：    期人效：

普拉提初阶2期验证组（xxx应用能力描述）服务周期：
目标·人服比：  复购率：    期人效：

普拉提初阶3期验证组（xxx应用能力描述）服务周期：
目标·人服比：  复购率：    期人效：
...
,t_link as (
    ...
    where
        camp_start_time > old_camp_start_time
...
,t_listen_detail as (
    select
        ...
        ,user_id
        -- 单课数量
        ,count(DISTINCT topic_name) filter(where now() >= topic_start_time) as topic_num
        ,sum(case when is_learn = '是' then 1 else 0 end) as learn_num
        ,sum(case when is_eff_learn = '是' then 1 else 0 end) as eff_learn_num
        ,sum(case when is_finish_learn = '是' then 1 else 0 end) as finish_learn_num
    from xqd_data_analysis.ads_xqd_study_user_listen_details_hour_bi_all
    where
        topic_type_name in ('视频', '直播', '智能直播')
        and topic_duration > 0
        and camp_name = '25天普拉提瘦身蜕变营'
...
,t_link_n as (
    select
         ...
         ,'销转第' || lpad(c.day_num::text, 2, '0') || '天' as sale_progress
         ,c.day_num
         ,d.agent_desc
    ...
    join (select generate_series(1, 100) as day_num) c
        on c.day_num <= (a.camp_end_time::date - b.sales_conversion_time::date + 1)
        and (b.sales_conversion_time::date + c.day_num - 1) <= current_date
...
,res as (
    select
        ...
        ,period_group as 期数分组
        ,period_name as 期数名称
        ,period_id as "期数ID"
        ,period_progress as 期数进度
        ,sale_progress as 销转进度
...
,res2 as (
    ...
    where
        -- 选择最新一天
        case when "Agent期数" is not null then 销转日期 + 销转进度序号 - 1 = current_date
        else 排序 <= 3 and 销转进度 in (
            select 销转进度
            from res
            where "Agent期数" is not null and 销转日期 + 销转进度序号 - 1 = current_date
        ) end
)
...
select
     ...
     ,string_agg(期数进度, ',' order by 结营日期 desc)
     ,string_agg(销转进度, ',' order by 结营日期 desc)
