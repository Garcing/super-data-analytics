...
,t_learn as (
    select
        ...
        ,json_object_agg(
         ...
         ) filter(where topic_name is not null) as finish_learn_rate_dict
    from (
        select
             a.period_id
            ,a.day_num
...
,t_question as (
    select
        ...
        ,json_object_agg(
         ...
         ) filter(where questionnaire_name is not null) as question_rate_dict
    from (
        select
             a.period_id
            ,a.day_num
...
,t_clock as (
    select
        ...
        ,json_object_agg(
            ...
            order by clock_time
         ) filter(where clock_task_name is not null) as clock_rate_dict
    from (
        select
             a.period_id
            ,a.day_num
...
,res as (
    ...
    select
         ...
         ,品类名称
         ,训练营名称
         ,null::text as "Agent期数"
         ,null::text as "Agent描述"
         ,null::text as "Agent目标人服比"
         ,期数分组
         ,实际期数进度
         ...
         ,sum(承接人数) / sum(售后接待数量) as 人服比
         ,sum(累计复购流水::bigint) / sum(售后接待数量) as 人效
         ...
         ,string_agg(完课率字典, ', ' order by 结营日期 desc) as 完课率字典
         ,string_agg(问卷提交率字典, ', ' order by 结营日期 desc) as 问卷提交率字典
         ,string_agg(打卡率字典, ', ' order by 结营日期 desc) as 打卡率字典
         ,max('否') as 是否播报期数
