...
,t_agent_desc as (
    ...
    union select 'Agent第二期', 'AI上新首版：Agent接量期/W1 SOP；AI外呼、360画像/问答、AI点评；MA接量期+W1/2，被动应答接量期+W1', '1:70'
    union select 'Agent第三期', 'AI迭代优化版：Agent接量期/W1 SOP；新增智群管家；MA扩展至W1/2/3/4，被动应答接量期+W1/2', '1:150'
    union select 'Agent第四期', 'Agent接量期/W1-W3 SOP迭代；AI工具同测试3期；MA接量期+W1/2/3/4，被动应答接量期+W1/2/3/4', '1:180'
)
,t_link as (
...
,t_window as (
    select *,row_number() over(partition by period_id, user_id order by is_recep desc) as period_rn,count(1) over(partition by period_id, sale_after_sys_user_name) as sale_after_recep_num from t_link
)
,t_link_n as (
    select a.*,b.sales_conversion_time,
    case when current_date < camp_start_time::date then '开营前第' || (camp_start_time::date - current_date) || '天' when current_date = camp_start_time::date then '开营日' when current_date > camp_start_time::date and current_date < sales_conversion_time::date then '开营后第' || (current_date - camp_start_time::date) || '天' when current_date = sales_conversion_time::date then '前置销转日' when current_date > sales_conversion_time::date and current_date < camp_end_time::date - 10 then '前置销转后第' || (current_date - sales_conversion_time::date) || '天' when current_date = camp_end_time::date - 10 then '结营日' when current_date > camp_end_time::date - 10 then '结营后第' || (current_date - camp_end_time::date) || '天' else '未知状态' end as cur_period_progress,
    case when b.sales_conversion_time::date + c.day_num - 1 >= sales_conversion_time::date and b.sales_conversion_time::date + c.day_num - 1 < camp_end_time::date - 10 then '前置期' when b.sales_conversion_time::date + c.day_num - 1 = camp_end_time::date - 10 then '结营日' when b.sales_conversion_time::date + c.day_num - 1 > camp_end_time::date - 10 then 'T+10' else '未知状态' end as period_progress,
...
,t_main as (
    select a.category_name,a.camp_name,a.period_name,a.period_id,a.cur_period_progress,a.period_progress,a.sale_progress,a.period_group,a.agent_group,a.agent_desc,a.agent_target_recep,a.camp_start_time,a.camp_end_time,a.sales_conversion_time,a.sale_after_sys_user_name,a.user_id,a.is_recep,a.is_refund,a.is_change_period_out,a.day_num,a.rn,
...
,res as (
    select category_name as 品类名称,camp_name as 训练营名称,agent_group as "Agent期数",agent_desc as "Agent描述",agent_target_recep as "Agent目标人服比",period_group as 期数分组,period_name as 期数名称,period_id as "期数ID",cur_period_progress as 最新期数进度,period_progress as 期数进度,sale_progress as 销转进度,camp_start_time::date as 开营日期,camp_end_time::date as 结营日期,sales_conversion_time::date as 销转日期,day_num as 销转进度序号,rn as 排序,
...
,res2 as (
    select 品类名称,训练营名称,"Agent期数","Agent描述","Agent目标人服比",期数分组,期数名称,最新期数进度 as 期数进度,销转进度,开营日期::text,结营日期::text,销转日期::text,承接人数,接待人数,复购人数,复购流水,售后接待数量,累计复购人数,累计复购主链路人数,累计复购回捞课人数,累计直播间复购人数,累计社群复购人数,累计前置期复购人数,累计结营日复购人数,"累计T+10复购人数",累计复购流水,承接人数 / 售后接待数量 as 人服比,累计复购流水::bigint / 售后接待数量 as 人效,round(累计复购人数 * 1.0 / 承接人数, 4) as 累计复购率,round(累计复购主链路人数 * 1.0 / 承接人数, 4) as 累计主链路复购率,round(累计复购回捞课人数 * 1.0 / 承接人数, 4) as 累计回捞课复购率,round(累计直播间复购人数 * 1.0 / 承接人数, 4) as 累计直播间复购率,round(累计社群复购人数 * 1.0 / 承接人数, 4) as 累计社群复购率,round(累计前置期复购人数 * 1.0 / 承接人数, 4) as 累计前置期复购率,round(累计结营日复购人数 * 1.0 / 承接人数, 4) as 累计结营日复购率,round("累计T+10复购人数" * 1.0 / 承接人数, 4) as "累计T+10复购率"
    from res where case when "Agent期数" is not null then 销转日期 + 销转进度序号 - 1 = current_date else 排序 <= 3 and 销转进度 in (select 销转进度 from res where "Agent期数" is not null and 销转日期 + 销转进度序号 - 1 = current_date) end
)
select '期数-明细' as 数据分组, * from res2 where "Agent期数" is not null
union
select '往期大盘-汇总',品类名称,训练营名称,null::text,null::text,null::text,期数分组,string_agg(期数名称,',' order by 结营日期 desc),string_agg(期数进度,',' order by 结营日期 desc),string_agg(销转进度,',' order by 结营日期 desc),string_agg(开营日期::text,',' order by 结营日期 desc),string_agg(结营日期::text,',' order by 结营日期 desc),string_agg(销转日期::text,',' order by 结营日期 desc),sum(承接人数),sum(接待人数),sum(复购人数),sum(复购流水),sum(售后接待数量),sum(累计复购人数),sum(累计复购主链路人数),sum(累计复购回捞课人数),sum(累计直播间复购人数),sum(累计社群复购人数),sum(累计前置期复购人数),sum(累计结营日复购人数),sum("累计T+10复购人数"),sum(累计复购流水),sum(承接人数) / sum(售后接待数量),sum(累计复购流水::bigint) / sum(售后接待数量),round(sum(累计复购人数) * 1.0 / sum(承接人数), 4),round(sum(累计复购主链路人数) * 1.0 / sum(承接人数), 4),round(sum(累计复购回捞课人数) * 1.0 / sum(承接人数), 4),round(sum(累计直播间复购人数) * 1.0 / sum(承接人数), 4),round(sum(累计社群复购人数) * 1.0 / sum(承接人数), 4),round(sum(累计前置期复购人数) * 1.0 / sum(承接人数), 4),round(sum(累计结营日复购人数) * 1.0 / sum(承接人数), 4),round(sum("累计T+10复购人数") * 1.0 / sum(承接人数), 4) from res2 where 期数分组 = '往期大盘' group by 1,2,3,4,5,6,7
