-- Simpler process data query for broadcast periods
with t_link_sub as (
    select period_id, day_num, count(1) as user_num
    from (
...
left join (
    select ls2.period_id, ls2.day_num, b.topic_name
        ,count(distinct b.user_id) filter(where is_finish_learn = '是') as finish_num
    from t_link_sub ls2
    left join xqd_data_analysis.ads_xqd_study_user_listen_details_hour_bi_all as b
        on ls2.period_id = b.period_id and b.topic_start_time::date <= (select act_date from t_link_sub where period_id = ls2.period_id and day_num = ls2.day_num)
