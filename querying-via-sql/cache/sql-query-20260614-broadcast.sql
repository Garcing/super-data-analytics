-- Broadcast query: step 1 - aggregate per period per day
with
t_period as (
    select * from (values
...
agg1 as (
    select
        ...
        sum(is_repurchase) as 当日复购人数,
        sum(order_money) as 当日复购流水
    from t_main
...
agg2 as (
    select
        *,
        sum(当日复购人数) over(partition by period_id order by 开营天数序号) as 累计复购人数,
        sum(当日复购流水) over(partition by period_id order by 开营天数序号) as 累计复购流水,
        sum(sum(当日复购人数) over(partition by period_id order by 开营天数序号)) over(partition by 实际期数进度) as 同期大盘累计复购人数,
        sum(sum(当日复购流水) over(partition by period_id order by 开营天数序号)) over(partition by 实际期数进度) as 同期大盘累计复购流水
    from agg1
)
select * from agg2