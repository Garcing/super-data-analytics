/*
 Part 1: 期数-明细 + Part 2: 往期大盘-汇总 (split from UNION)
*/
...
,t_learn as (
    select period_id, day_num
        ,json_object_agg(topic_name, round((finish_learn_num * 1.0 / user_num), 4) order by topic_start_time) as finish_learn_rate_dict
    from (
        select a.period_id, a.day_num, a.user_num, b.topic_name, b.topic_start_time
        ...
        left join xqd_data_analysis.ads_xqd_study_user_listen_details_hour_bi_all as b
            on a.period_id = b.period_id and b.topic_start_time::date <= act_date and b.topic_finish_time < now()
        group by 1,2,3,4,5
    ) as t group by 1,2
)
,t_question as (
    select period_id, day_num
        ,json_object_agg(questionnaire_name, round((question_num * 1.0 / user_num), 4) order by question_visit_create_time) as question_rate_dict
    from (
        select a.period_id, a.day_num, a.user_num, b.questionnaire_name
        ...
        left join xqd_data_analysis.ads_xqd_platform_question_user_questionnaire_detail_hour_bi_all as b
            on a.period_id = b.period_id and b.question_visit_create_time::date <= act_date and b.question_visit_create_time < now()
        group by 1,2,3,4
    ) as t group by 1,2
)
,t_clock as (
    select period_id, day_num
        ,json_object_agg(clock_task_name, round((clock_num * 1.0 / user_num), 4) order by clock_time) as clock_rate_dict
    from (
        select a.period_id, a.day_num, a.user_num, b.clock_task_name
            ,min(b.clock_time) as clock_time, count(distinct b.user_id) as clock_num
        from t_link_sub as a
        left join xqd_data_analysis.ads_xqd_study_user_clock_detail_hour_bi_all as b
            on a.period_id = b.period_id and b.clock_time::date <= act_date and b.clock_time < now()
        group by 1,2,3,4
    ) as t group by 1,2
)
,t_main as (
    select
         a.order_id, a.category_name, a.camp_name, a.period_name, a.period_id
        ,a.cur_period_progress, a.period_progress_sale_section, a.period_progress
        ,a.period_group, a.agent_group, a.agent_desc, a.agent_target_recep
        ,a.sale_after_wework_user_id, a.camp_start_time, a.camp_end_time, a.sales_conversion_time
        ,a.sale_after_sys_user_name, a.user_id, a.is_recep, a.is_refund, a.is_change_period_out
        ,a.day_num, a.rn
    ...
    left join xqd_data_analysis.ads_xqd_revenue_ord_all_order_detail_bi as b
        on a.user_id = b.user_id and a.is_recep = 1
...
,res1 as (
    select
        ...
        ,agent_group as agent_period, agent_desc as agent_desc, agent_target_recep as agent_target_recep
        ,case
        ...
        ,period_id as period_id, cur_period_progress as cur_period_progress
        ,period_progress as period_progress, period_progress_sale_section as period_progress_sale_section
        ,camp_start_time::date as camp_start_date, camp_end_time::date as camp_end_date
        ,sales_conversion_time::date as sales_conversion_date
        ,day_num as day_num, rn as rn
...
-- Only 期数-明细 (no UNION, no 往期大盘-汇总)
select
     '期数-明细' as data_group
    ,品类名称, 训练营名称, agent_period, agent_desc, agent_target_recep
    ,period_group, period_progress, period_name, cur_period_progress, period_progress_sale_section
    ,camp_start_date::text, camp_end_date::text, sales_conversion_date::text
    ,accept_count, recep_count, repurchase_count, repurchase_revenue, sale_after_count
    ,cum_repurchase_count, cum_main_rep_count, cum_back_rep_count
    ,cum_live_repurchase_count, cum_group_repurchase_count
    ,cum_pre_repurchase_count, cum_end_repurchase_count, cum_t10_repurchase_count, cum_repurchase_revenue
