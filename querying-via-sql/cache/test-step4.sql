-- Test: res1 + t_link_sub + t_learn join
with t_period as (
    select * from (values
...
,t_agent_desc as (
    select 'Agent第一期' as agent_group, '验证运营策略' as agent_desc, '1:120' as agent_target_recep
    union select 'Agent第二期', 'AI上新', '1:70'
)
,t_link as (
...
,t_link_n as (
    ...
    join (select generate_series(1, 100) as day_num) c
        on c.day_num <= (a.camp_end_time::date - a.camp_start_time::date + 1) and (a.camp_start_time::date + c.day_num - 1) <= current_date
...
,res1 as (
    select
        ...
        ,count(distinct sale_after_sys_user_name) as sale_after_count, count(1) as accept_count
        ,sum(is_repurchase) as repurchase_count, sum(order_money) as repurchase_revenue
...
-- Just test joining t_learn with res1 for Agent第二期 periods on 开营第04天
select a.period_name, a.period_progress, b.period_id, b.day_num, b.finish_learn_rate_dict
from res1 a
left join (
...
) b on a.period_id = b.period_id and a.day_num = b.day_num
where a.agent_period = 'Agent第二期' and a.period_progress = '开营第04天'
