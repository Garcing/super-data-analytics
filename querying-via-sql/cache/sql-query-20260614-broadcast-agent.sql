-- Simplified version for 2026-06-14 broadcast
with
t_period as (
    select * from (values
...
select
    period_name as 期数名称,
    coalesce(agent_group, '') as "Agent期数",
    coalesce(agent_desc, '') as "Agent描述",
    coalesce(agent_target_recep, '') as "Agent目标人服比",
    coalesce(
        case
    ...
    sum(is_repurchase) as 本日复购人数,
    sum(order_money)::bigint as 本日复购流水
from t_main
