...
,t_learn as (
    select
        ...
        ,string_agg(
            concat('"', topic_name, '": ', round((finish_learn_num * 1.0 / user_num), 4)::text),
...
,t_question as (
    select
        ...
        ,string_agg(
            concat('"', questionnaire_name, '": ', round((question_num * 1.0 / user_num), 4)::text),
...
,t_clock as (
    select
        ...
        ,string_agg(
            concat('"', clock_task_name, '": ', round((clock_num * 1.0 / user_num), 4)::text),
            ', ' order by clock_time
         ) as clock_rate_dict
    from (
        select
...
,res2 as (
    select
        ...
        -- 过程数据
        ,concat('"', 期数名称, '": { ', coalesce(b.finish_learn_rate_dict, ''), ' }') as 完课率字典
        ,concat('"', 期数名称, '": { ', coalesce(c.question_rate_dict, ''), ' }') as 问卷提交率字典
        ,concat('"', 期数名称, '": { ', coalesce(d.clock_rate_dict, ''), ' }') as 打卡率字典
...
select "数据分组", "品类名称", "训练营名称", "Agent期数", "Agent描述", "Agent目标人服比", "期数分组", "实际期数进度", "期数名称", "最新期数进度", "期数销转区间", "开营日期", "结营日期", "销转日期", "承接人数", "接待人数", "复购人数", "复购流水", "售后接待数量", "累计复购人数", "累计复购主链路人数", "累计复购回捞课人数", "累计直播间复购人数", "累计社群复购人数", "累计前置期复购人数", "累计结营日复购人数", "累计T+10复购人数", "累计复购流水", "人服比", "人效", "累计复购率", "累计主链路复购率", "累计回捞课复购率", "累计直播间复购率", "累计社群复购率", "累计前置期复购率", "累计结营日复购率", "累计T+10复购率", "完课率字典", "问卷提交率字典", "打卡率字典", "是否播报期数" from res