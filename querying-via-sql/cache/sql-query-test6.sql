...
res1 as (
    select
        ...
        count(distinct sale_after_sys_user_name) as 售后接待数量, count(1) as 承接人数,
        sum(is_repurchase) as 复购人数, sum(order_money) as 复购流水,
...
res2 as (
    select
         品类名称, 训练营名称, "Agent期数", "Agent描述", "Agent目标人服比",
         期数分组, 实际期数进度, 期数名称, 最新期数进度, 期数销转区间,
         开营日期::text as 开营日期, 结营日期::text as 结营日期, 销转日期::text as 销转日期,
         承接人数, 复购人数, 复购流水, 售后接待数量,
         累计复购人数, 累计直播间复购人数, 累计社群复购人数,
         累计前置期复购人数, 累计结营日复购人数, "累计T+10复购人数", 累计复购流水,
         承接人数 * 1.0 / 售后接待数量 as 人服比,
         ...
         case when "Agent期数" is not null and 开营日期::date + 开营天数序号 - 1 = current_date then '是' else '否' end as 是否播报期数
    from res1
    where ("Agent期数" is not null and 开营日期::date + 开营天数序号 - 1 = current_date)
)
select * from res2