...
### 3.1 代码就位
代码位于本仓库根目录。服务器部署目录 `~/sda-mcp/`：
```bash
git archive --format=tar.gz HEAD -o mcp.tgz
# scp 到服务器后：
mkdir -p ~/sda-mcp && tar -xzf mcp.tgz -C ~/sda-mcp
```
...
```bash
scp sda_mcp/<file>.py hermes:~/sda-mcp/sda_mcp/<file>.py
...
- **回退**：从目标旧提交重新执行 `git archive` → 覆盖服务器代码 → 重建。Neo4j 数据不丢（除非 sync 重灌）。
...
5. 跑完整测试：`python -m pytest -q`，再用真实 Neo4j 做代表性冒烟；测试通过不等于检索质量通过。
...
- **独立仓库**：本仓库只维护 SDA MCP 服务；拆分前的 CLI Skill 套件已归档到 [`Garcing/super-data-analytics-old`](https://github.com/Garcing/super-data-analytics-old)。
- **agent 技能文档**：`skills/`（入口 `skills/README.md`，9 个技能的 SKILL.md + references，指导 agent 使用 19 个工具），与 Python 执行内核 `sda_mcp/skills/` 分层——一个给 LLM 读，一个是代码。
- **测试**：`python -m pytest -q`（当前 164 通过 + 7 个集成测试跳过）。mock 单元测试 + 可选真实服务集成测试（`SDA_INTEGRATION=1`）。
- **设计记录**：Neo4j / GraphRAG 的当前取舍见 `docs/neo4j-graphrag-2026-research.md`；历史设计文档保存在旧仓库。
- **MCP 设计参考**：遵循 MCP 官方文档与 `mcp-builder` 技能。
