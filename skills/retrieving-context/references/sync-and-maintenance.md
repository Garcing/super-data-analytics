...
真相源：`sda_mcp/skills/retrieving_context_sync.py`。`sync` 只有一个参数 `dry_run: bool=false`；不要传未声明字段。由于默认值会执行破坏性全量重建，**先明确传 `dry_run=true` 预检再全量**，不要裸调。
...
- 服务器上 sync 属长任务，建议重建容器清掉进程内模型缓存后再跑（详见仓库根目录 `README.md` §6）。
