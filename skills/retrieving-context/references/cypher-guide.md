...
- **每命中、每类邻居标签应用两道门，任一超限整桶丢弃 items**：总数不超过 10、且桶内 `items` 累计 UTF-8 序列化字节不超过服务端字节预算（4KB 量级）时，返回按邻居实体 `key_field` 排序的全部邻居；否则返回 `items=[]`、真实 `total`、`truncated=true` 和 `omitted_reason`——超数报 `"high_cardinality"`（双超限优先），仅超字节报 `"byte_budget"`。自环关系（from=to）双向扩展且排除自身。
