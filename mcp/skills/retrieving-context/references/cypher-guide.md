...
真相源：`mcp/sda_mcp/skills/retrieving_context.py`（`schema` / `cypher` / `fetch_graph_context_batch`）。
...
6. 邻居太多时先 `LIMIT`：检索结果的 context 已截断，完整遍历（如展开整棵业务层级）用 Cypher 并限制返回量。
...
- **每命中、每类关系最多 20 个邻居**：按 graph-config 声明的关系及方向批量查询，同一命中同一邻居标签最多保留 20 条。自环关系（from=to）双向扩展且排除自身。

因此检索结果 `context` 里邻居不全属正常现象：某指标关联 50 张表时只会看到前 20 张。需要完整邻居（计数、全量列举、递归层级）时，改用 `retrieve_schema` + `retrieve_cypher` 自己写遍历，例如：
