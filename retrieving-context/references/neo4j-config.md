...
```json
"指标": { "table_id": "tblDiMrN7GaGVtpc", "key_field": "指标名称" },
"数据看板": { "table_id": "tblaOZqdUnySDDMG", "key_field": "数据看板名称", "vector_index": false }
```
...
| `vector_index` | 否 | 是否建向量索引、参与语义检索。默认 `true`。设 `false` 的实体（如 数据看板、数据域）只通过关系被扩到，不直接被 `--question` 向量命中。 |

当前 8 个实体：业务线、业务板块、业务小点、指标、维度、数据看板（`vector_index:false`）、数据域（`vector_index:false`）、表。
...
- 业务小点→指标：外键在 from 侧（业务小点表有个多选"指标名称"字段）

**多选字段自动展开**：飞书多选字段（list）会被 `_expand_records` 展开成多行，每个值单独参与匹配，所以一对多关系不用特殊配置，照常写 `match`。
...
当前 7 条关系：6 条 `match`（业务线→板块→小点→指标、看板→指标、维度→指标、数据域→表），1 条 `via`（表↔表 关联）。
